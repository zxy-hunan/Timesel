package com.hunan_zxy.timelib.utils;

import android.annotation.SuppressLint;

import com.hunan_zxy.timelib.entity.DayEntity;
import com.hunan_zxy.timelib.entity.MonthBean;
...
public class BhUtils {
     ...
     * @return
     */
    public static ArrayList<MonthEntity> getMonthList(){
        ArrayList<MonthEntity> list = new ArrayList<>();
        for (int i = 1; i <= 12; i++) {
            list.add(new MonthEntity(i+"月"));
        }
        return list;
    }


    public static ArrayList<DayEntity> getMonthDay(int year, int month, int day) {

        ArrayList<DayEntity> fullDayList = new ArrayList<DayEntity>();
        if (day <= 0) day = 1;
        Calendar cal = Calendar.getInstance();// 获得当前日期对象
        cal.clear();// 清除信息
        cal.set(Calendar.YEAR, year);
        ...
        int count = cal.getActualMaximum(Calendar.DAY_OF_MONTH);//当前日期的最后一天
        for (int j = 0; j <= (count - 1); ) {

            cal.add(Calendar.DAY_OF_MONTH, j == 0 ? +0 : +1);
            j++;

            fullDayList.add(new DayEntity(j+"日"));
        }
        return fullDayList;
    }
